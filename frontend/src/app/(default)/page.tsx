...
export default async function SayfamizdaPage() {
  ...
  // Try to use server-provided cookie snapshot to avoid an extra backend
  ...
  if (user?.role === 'super_admin') {
    ...
    try {
      ...
      const initialCompaniesRaw = cookieStore.get('initialCompanies')?.value ?? null;
      if (initialCompaniesRaw) {
        const parsed = JSON.parse(initialCompaniesRaw);
        companiesResult = { status: 'success' as const, data: parsed };
      } else {
