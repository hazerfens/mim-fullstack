...
type RolePermission struct {
	...
	Domain     string         `gorm:"type:varchar(100);default:'*'" json:"domain,omitempty"`
...
func NewRolePermission(roleID uuid.UUID, resource, action, effect, domain string, conditions []byte, priority int, isActive bool) RolePermission {
	rp := RolePermission{
		RoleID:   roleID,
		Resource: resource,
		Action:   action,
